...
| 6 | **TAB 구분자 회귀(기존 결함 #22985, 2026-08-25 재설계)** — 사용자가 제공한 결함 티켓(상태=보류, Grade D, 발생 버전 1.0.11.014, 연구소 등록·인지)과 사양서4(260824) VP-688 재확인 근거: Save Sample **파일**의 구분자는 Save-As 대화상자의 파일 형식 선택이 정하고, **Import 파싱**이 인식하는 구분자는 화면의 Data Delimiter(Input Format) 콤보가 정한다 — 서로 다른 컨트롤이 서로 다른 것을 결정한다. Save-As 자동화(owner-draw 콤보)에는 더 이상 의존하지 않고, 이미 확보한 COMMA 헤더로 TAB 파일을 코드가 직접 만든다. ①티켓의 결함 재현 경로(Comma→Tab **직접** 전환)와 ②티켓에 적힌 워크어라운드(반대값을 한 번 거쳤다가 목표값으로 옴) 둘 다 실행해, TAB 설정 상태에서 TAB 파일과 COMMA 파일(불일치) 각각의 Import 미리보기 결과를 비교한다. 항상 COMMA로 원복 | 사양대로면 TAB 파일 성공+COMMA 파일 실패(PASS). 티켓의 결함 패턴과 정확히 일치하면(TAB 실패+COMMA 성공) 그 사실을 티켓 번호와 함께 note에 남기고 FAIL(신규 결함 아님을 명시). 둘 다 아니면 MANUAL | PASS/FAIL/MANUAL |
| 7~11 | Service Manual 4.6.7절/사양서1 VP-474 근거: "Import Patient Order"(수동)와 "Use Import Patient Information From a Specific Folder"(폴더 자동 감지)는 상호 배타 기능이다 — 후자를 켜면 Reserved의 수동 Import 버튼(30392)이 사라지고 Auto Patient Import 버튼(30671)+Patient Import Status(30021)가 나타난다. Yes/No 라디오 확인 → Yes로 켜기 → Target Directory 지정 → Update → 버튼 교체 확인 → Search로 폴더 1회 스캔해 DB(`ORDER_PATIENT`) 반영 확인 → **항상 No로 원복** → 수동 Import 버튼 재노출 확인까지 라이브로 전부 수행한다(2026-08-21 옵트인 최초 구현 → 2026-08-25 기본 실행 경로로 라이브 전 구간 검증 완료, `--with-folder-watch` 플래그 불필요) | Yes 전환·Target Directory 반영·버튼 교체·DB 반영·No 원복·버튼 복귀가 전부 확인돼야 한다 | PASS/FAIL |
| 12 | Column Mapping — 필수 아닌 컬럼(예: Institution Name, 사양서4 p.60 VP-688 "필수(체크 해제 불가)" 표 기준) 하나를 선택 해제 → Update → Save Sample로 헤더에서 실제로 빠졌는지 확인 → 축소된 헤더로도 Sample Test 파싱이 정상 동작하는지 확인 → 원래 체크 상태로 원복 | 선택 해제/원복 반영 확인 + 축소 헤더로도 파싱 성공 | PASS/FAIL |

**TC13의 MANUAL 2건(2026-08-25 라이브 관찰)**: TAB 구분자 회귀의 두 경로(직접 전환/워크어라운드) 모두 **TAB 파일과 COMMA 파일이 둘 다 Import 성공**했다 — 이는 사양(불일치 시 실패해야 함)과도, 티켓 #22985의 정확한 결함 패턴(Tab 실패/Comma 성공, 반대)과도 다르다. 완전히 깨진 파일로는 미리보기가 실제로 0행을 보여주는 것도 별도로 확인해(그리드가 이전 결과를 그대로 남기는 자동화 함정이 아님) 이 관찰은 실제 제품 동작으로 보인다. **주목할 점**: 티켓의 발생 버전은 1.0.11.014이고 이 실행의 VXvue는 1.0.11.015 — 두 버전 사이에 이 기능의 동작이 바뀌었을 가능성이 있다(부분 수정 또는 다른 형태의 회귀). 새 결함으로 단정하지 않고 사람 확인이 필요한 MANUAL로 남기되, 이 버전 간 차이는 QA/연구소에 전달할 가치가 있다.
...
13. **모든 Step은 비개발자가 읽어도 이해할 수 있게 출력한다.** 리포트에는
    시험 목적, 수행 내용, 합격 기준, 확인 결과, 판정 이유, 후속 조치를 빠짐없이
    표시한다. 짧은 내부 Step명은 `core/report_language.py`의 사용자용 문장과
    연결하며, 새 Step을 추가할 때 문장 사전도 같은 변경에서 갱신한다.
14. **실제 결과를 추측해 채우지 않는다.** 화면·DB·파일·로그에서 정확한 값을
    얻을 수 있으면 그대로 남긴다. 얻을 수 없으면 `자동으로 실제 결과를 확정할
    수 없었다. 기대 결과와 같은지 사용자가 직접 확인해야 한다.`라고 표시하고
    MANUAL과 확인 방법을 함께 남긴다.
