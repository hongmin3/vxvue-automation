...
## 12. 이후 TC에도 적용할 리포트 언어 기준

이 설계에서 새 TC나 Step을 만들 때는 짧은 코드 제목만 추가하지 않는다.
`core/report_language.py`에 비개발자가 이해할 수 있는 수행 문장을 함께 등록하고,
리포트의 시험 목적·수행 내용·합격 기준·확인 결과·판정 이유·후속 조치가 모두
채워져야 한다. 정확한 실제값을 자동 수집할 수 없으면 추측하지 않고 기대 결과와
같은지 사용자 확인이 필요하다고 명시한다. FAIL/MANUAL/SKIP/BLOCKED는 리포트
상단 확인 목록에도 노출하며, 단독 TC와 전체 회귀에 동일하게 적용한다.
