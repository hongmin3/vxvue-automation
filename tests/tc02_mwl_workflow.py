...
DB_MISSING_NOTE = (
    '**표시되지 않았다(실측, 2026-08-19).** Operation Manual 3.6(p.41): "Database 모드에서 완료된 검사 목록 및 영상을 조회"한다 - 즉 완료되지 않은 검사는 표시되지 않는다. 이번 실행은 사양서1 p.38의 \'Procedure Mapping 하지 않고 Exposure Mode로 전환\'을 택했기 때문에 **Step이 등록되지 않았고**, DB의 STUDY.StudyStatus가 1로 남았다(전날 정상 종료된 검사는 0). Operation Manual 6.8(p.99)이 말하는 "촬영 예정인 Step이 남아 있는 경우 검사를 닫거나 보류"의 보류 상태로 보인다. 즉 이것은 제품 결함이 아니라 **자동화가 매핑을 생략한 결과**일 가능성이 높다 - 확정하려면 Procedure Mapping을 수행한 뒤 같은 흐름을 다시 돌려 비교해야 한다(사용자 확인 필요: 매핑은 제품 설정을 바꾸는 조작이라 자동으로 하지 않았다). 위 DB 대조 Step은 정상 통과했으므로 \'촬영 결과가 저장됐다\'는 사실 자체는 확인됐다.'
)
