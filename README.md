...
**해결** — 판정 근거를 값으로 옮기고, 기준자료 서명을 둘로 나눴다.

| 서명 | 포함 | 쓰임 |
|---|---|---|
| 구조 서명 | 라이선스, 연동 상태, 메뉴 목록 (**테마·폰트 제외**) | 값 JSON 비교 — **주 판정 근거** |
| 외형 서명 | 위 + 테마 + 폰트 + 본문 창 크기 | 캡처(SSIM) 비교 — 같은 테마끼리만 |
