...
| 실행 | 판정 합계 | 소요 |
|---|---|---|
| 2026-08-20 15:24 | PASS 69 / FAIL 5 / MANUAL 14 / SKIP 5 / BLOCKED 0 | 39분 27초 |

**이 수치는 낡았다.** 그 뒤 FAIL 5건의 원인(TC04가 XIPL Studio를 닫지 않아 이후
TC의 DICOM 전송이 막힘)과 TC07/TC08의 판정 결함을 고쳤고, 개별 TC 단독 실행으로는
확인했지만 **전체 회귀를 다시 돌리지는 않았다.** 지금 상태는
`automation_scope.json`의 TC별 `level`과 `Reports/`의 최근 리포트를 보는 것이
정확하다.
