...
## 0. 5분 온보딩 — 이것만 먼저 읽으면 된다

아래 절들은 각각 깊게 들어가므로, 처음 보는 사람은 이 절만 읽고 필요한 곳으로
건너뛰면 된다.

**무엇인가**: 의료영상 뷰어(VXvue)의 Windows 업데이트 호환성 검증 체크리스트
15개 TC를 무인 실행 → 자동 판정 → 사용자용 증거 리포트까지 만드는 QA 자동화다.
Python 4개 의존성 + Win32 API만 쓰고, 판정 근거는 화면 캡처가 아니라 **DB 대조·
제품 로그·Win32 메시지**에서 가져온다.

**설계의 한 줄**: "헛된 PASS를 만들지 않는다" — 클릭 성공을 결과로 쓰지 않고,
자동 판정이 불가능한 항목은 PASS로 밀지 않고 **MANUAL로 정직하게 남긴다**
(→ 2절).

**지금 어디까지 됐나** (`automation_scope.json`이 유일한 근거):

| 수준 | TC |
|---|---|
| FULL (자동 판정 완결) | TC02 · 06 · 07 · 08 · 11 · 13 |
| PARTIAL (일부 MANUAL 남음) | TC03 · 04 · 05 · 12 · 14 |
| MANUAL (사람이 판정) | TC09 |
| EXCLUDED (자동화 범위 밖) | TC01 · 10 · 15 |

미구현 TC는 없다. 검증 범위는 **DX(일반촬영) 전용**이며, 이 제한은 모든 리포트
상단에 자동으로 표시된다.
...
**관리자 권한 + DPI 100%가 필수다.** 일반 권한이면 캡처는 되고 클릭만 조용히
실패해 엉뚱한 증상으로 보인다 — `preflight`가 NG로 막는다. 다른 PC에서 돌릴 때의
전제와 남은 제한은 `docs/PORTABILITY_AUDIT.md`에 따로 정리했다.

**코드 구조 한 장** (상세는 8절):

```
run.py                 CLI (20여 개 명령 — tcNN 단독 실행 / run-regression / 조사 명령)
core/                  공통 인프라: ui(Win32 조작) · setting(Setting 화면) ·
                       db(DRF 대조) · result(판정·리포트) · regression(전체 회귀 러너) ...
tests/tcNN_*.py        TC 하나 = 파일 하나. 모듈의 TC_ID가 체크리스트 행과 맵핑된다
automation_scope.json  TC별 자동화 수준 + 그 판단 근거(근거 없이 수준을 못 바꾼다)
docs/                  검증 명세 · 설계 기록 · 이식성 점검 (전부 마스킹 사본)
Reports/ Evidence/     실행 결과와 증거 (gitignore — 실행하면 다시 생긴다)
```
...
| 알고 싶은 것 | 볼 곳 |
|---|---|
| 왜 이렇게 설계했나, 무엇에 당했나 | 2절, 4절(실전 문제와 해결) |
| 지금 무엇이 검증되는가 | 3절, `automation_scope.json` |
| 어떻게 돌리나 | 5절 |
| 각 TC가 코드상 어떤 Step을 밟나 | `docs/TC_검증상세.md` |
| 다른 PC에서 돌릴 수 있나 | `docs/PORTABILITY_AUDIT.md` |
| 결과 리포트를 어떻게 읽나 | 11절 |
...
| 명령 | 내용 |
|---|---|
| `python run.py selfcheck` | **자동화 자신을 검사한다** — 저장소 정적 일관성 10종 + 단위테스트. 제품·DB·시험 서버를 건드리지 않으므로 시험 PC가 아닌 곳에서도 돌아간다. 전체 회귀 전에 먼저 돌린다(아래 5.8) |
...
### 5.8 끝났다는 것을 놓치지 않기 — 자기 검사와 종료 알림

전체 회귀는 25~40분이 걸린다. 그동안 자리를 비우거나 다른 창을 보고 있으면,
**끝났는지·통과했는지**를 알 방법이 긴 로그를 되짚는 것밖에 없었다. 실제로
백그라운드 회귀가 두 차례 중간에 끊겼고, 끊긴 것을 늦게 알아 그만큼 검증이
밀렸다. 두 층으로 나눠 해결했다.

**① 돌리기 전에 — `python run.py selfcheck`**

제품을 켜지 않고 자동화 자신만 검사한다(몇 초). 회귀를 25분 돌린 **뒤에야**
드러나는 종류의 오류를 앞으로 당긴다.

| 검사 | 무엇을 막나 |
|---|---|
| TC ID 삼중 일치 (`automation_scope.json` ↔ `core/regression.IMPLEMENTED` ↔ `tests/*.TC_ID`) | 실행은 성공하는데 **리포트의 TC ID와 실제 실행된 코드가 달라** 체크리스트 결과가 엉뚱한 행에 들어가는 것. 가장 발견이 늦는 오류다 |
| 파일명 번호 ↔ TC_ID 일치 | 같은 원인 |
| scope의 `level`·`reason` 존재, 값 유효성, 중복 없음 | 근거 없이 자동화 수준이 기록되는 것 |
| `TC_PURPOSES`·`TC_LABELS` 커버리지 | 리포트에 시험 목적이 비는 것 |
| `COMMANDS`의 함수 존재 | 등록만 되고 없는 명령 |
| 판정 규칙 단위테스트 | `TCResult.verdict`가 조용히 바뀌어 **모든 TC 판정이 함께 틀어지는 것** |
| 리포트 완전성 게이트 테스트 | 사용자용 문장이 빈 리포트가 나가는 것 |
| 크래시 덤프 `since` 필터 테스트 | **예전 덤프를 이번 크래시로 오인**해 정상 종료를 크래시로 보고하는 것 |

정적 검사는 TC 모듈을 **import하지 않는다** — `TC_ID = "..."` 줄을 소스에서
읽는다. 무거운 의존성 없이 어디서든 돌게 하려는 것이다. 그리고 검사 자체가
빈 집합끼리 비교해 통과하지 않도록, **일부러 어긋난 값을 넣어 각 검사가 실제로
문제를 잡는지**까지 테스트한다.

**② 끝난 뒤에 — 종료 알림**

모든 실행 명령이 끝에 결과 배너를 찍고, **별도 창 하나**를 띄운다. 판정에 따라
문구가 갈린다(`완료 — 전부 통과` / `실패 — FAIL 있음` / `완료 — 사람이 확인할
항목 있음` / `비정상 종료`). 예외로 죽거나 Ctrl+C로 끊긴 경우도 **조용히 사라지지
않고** `비정상 종료`로 알린다 — 중단인지 완료인지 나중에 구분할 수 없었던 것이
문제였다.

설계상 지킨 것: **알림이 판정을 바꾸지 않는다.** 알림 자체가 실패해도 예외를
밖으로 내보내지 않고, 창은 비차단(detached)이라 아무도 닫지 않아도 명령이 끝나고
종료 코드가 나간다(예약 실행에서 팝업이 프로세스를 붙들지 않게). 창은 포커스를
가져가므로 **UI 자동화가 다 끝난 뒤에만** 띄운다. 창이 방해되면 `--no-popup`
(배너는 그대로 찍힌다).
...
"delay 없이"), 그리고 제품 동작이 사양·결함 티켓과 모두 다르게 관찰돼 사람의
판단이 필요한 항목(TC13의 TAB 구분자)이다. **추측으로 PASS를 만들지 않는다**는
설계 원칙(2절)이 그대로 드러나는 숫자다.

주목할 점 하나: `TC_WindowsUpdate_14`는 SKIP이 있는데도 판정이 PASS다. `--deep`
미수행 Step은 "PASS를 막지 않는다"고 명시적으로 확정한 **유일한 예외**이고, 그
규칙은 단위테스트로 고정돼 있다(5.8절).
...
│  ├─ test_verdict.py         판정 규칙 (TCResult.verdict)
│  ├─ test_report_quality.py  리포트 완전성 게이트
│  ├─ test_crash.py           크래시 덤프 since 필터
│  ├─ test_notify.py          종료 알림 합산·문구
│  └─ test_static.py          정적 검사가 실제로 문제를 잡는지
└─ tests/
...
| `docs/PORTABILITY_AUDIT.md` | **다른 PC에서 실행할 때의 전제와 남은 제한**. `config.example.json`의 키가 코드에서 실제로 어떻게 쓰이는지 따라가 확인한 결과 — 차단 조건(관리자 권한·DPI·실행 파일·DB)과 아직 이 시험 PC 경로가 남은 자리를 구분해 둔다 |
...
작업 인수인계·현재 상태·다음 작업 큐(`HANDOFF.md` / `CURRENT_STATE.md` /
`NEXT_TASK.md`)와 판단 기준 문서(`CLAUDE.md`)는 이 저장소 상위 폴더(`VXvue/`)에서
관리한다. 사내 실측 기록·사양 원문 인용이 섞여 있어 이 공개 저장소에는 포함하지
않는다.

**사양서·매뉴얼 원본(비공개 `VXvue 지식파일/`)의 버전이 바뀌면, 그 사실을 알게 된
같은 작업에서 `CLAUDE.md` 4절의 버전 표기도 함께 갱신한다** — 나중으로 미루지
않는다. 실제로 `260820`을 최신으로 기록해 둔 사이에 원본이 두 번 더 갱신돼
`260824`가 돼 있던 것을 몰랐던 일이 있었다(2026-08-25 발견). 판정의 1차 근거가
구버전 사양이 되는 것을 막기 위한 규칙이다.

## 11. 사용자용 결과 리포트 작성 기준

리포트는 자동화 개발자가 아니라 시험 결과를 판단하는 사용자가 먼저 읽는다.
따라서 전체 회귀와 `python run.py tcNN` 같은 단독 실행은 동일한 품질로 다음을
항상 표시한다.

- TC의 시험 목적과 각 Step의 구체적인 수행 내용
- 합격 기준, 자동으로 확보한 실제 결과, PASS/FAIL/MANUAL/SKIP/BLOCKED 판정 이유
- 실패·확인 필요·미수행 항목의 후속 조치와 리포트 상단 요약
- MWL, DB, SCP, OCR 같은 기술 용어의 사용자용 설명
...
문장을 등록하고 `core.result.assert_report_readable()`에서 미등록 Step과 빈
사용자용 필드가 0건인지 확인해야 완료로 본다.
